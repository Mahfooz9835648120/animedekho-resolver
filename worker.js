...
async function extractAnimeSaltPageServers(slug){
 try{
  if(!slug)return [];
  const candidates=[
  ...
  const out=[];const seen=new Set();
  const add=(raw,name='AnimeSalt')=>{
   if(!raw)return;
   const u=absUrl(ANIMESALT_SITE,raw);
   if(!/^https?:/i.test(u)||seen.has(u))return;
   if(/animesalt\.link\/episode\//i.test(u))return;
   seen.add(u);
   const lower=`${name} ${u}`.toLowerCase();
   const type=detectType(lower,'multi');
   out.push({provider:'AnimeSalt',name:name||'AnimeSalt',type,embedUrl:u});
  };
  for(const target of candidates){
   const r=await fetch(target,{headers:{'User-Agent':UA,'Accept':'text/html,application/xhtml+xml'}});
   if(!r.ok)continue;
   const html=await r.text();
   for(const m of html.matchAll(/<iframe[^>]+(?:src|data-src|data-url|data-embed|data-iframe|data-player)=[\"']([^\"']+)[\"'][^>]*>/gi))add(m[1],'Iframe');
   for(const m of html.matchAll(/(?:data-(?:src|url|embed|iframe|player)|(?:src|href))=[\"']([^\"']+)[\"']/gi))add(m[1],'Iframe');
   for(const m of html.matchAll(/(?:https?:)?\/\/[^\"'<>\s]+/gi)){
    const u=m[0];
    if(/(iframe|embed|player|mystream|abyss|playx|megacloud|vidstream)/i.test(u))add(u,'Detected Embed');
   }
   if(out.length)break;
  }
  return out;
...
async function handleResolve(request){const {searchParams,origin}=new URL(request.url);const slug=searchParams.get('slug');const anilistId=searchParams.get('anilistId')||'';const ep=searchParams.get('ep');const season=searchParams.get('season');const saltId=searchParams.get('saltId');
 ...
 if(!isMovie){
  ...
  if(effectiveSaltId&&effectiveEp){
   ...
   saltRequests.push(fetchJSON(anilistId?`${base}&anilistId=${encodeURIComponent(anilistId)}`:base));
  }
  saltRequests.push(extractAnimeSaltPageServers(slug));
  const [saltApi,saltPage]=await Promise.all(saltRequests);
  const saltServers=[...(Array.isArray(saltApi?.servers)?saltApi.servers:[]),...(Array.isArray(saltPage)?saltPage:[])];
  for(const s of saltServers){
   const url=s?.url||s?.embedUrl||s?.embed||s?.iframe||s?.src||s?.link;
   if(!url)continue;
   const name=s?.name||s?.title||s?.server||'AnimeSalt';
   if(!results.servers.some(x=>x.embedUrl===url))results.servers.push({provider:'AnimeSalt',name,type:detectType(name,'multi'),embedUrl:url});
  }
 }
